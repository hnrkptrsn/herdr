...
pub(crate) const VIBE_CONFIG_DIR_ENV_VAR: &str = "VIBE_CONFIG_DIR";
...
pub(crate) fn vibe_dir() -> io::Result<PathBuf> {
    // VIBE_CONFIG_DIR is a herdr-level override only (primarily a test
    // seam); Vibe CLI uses ~/.vibe by default.
    if let Some(value) = std::env::var_os(VIBE_CONFIG_DIR_ENV_VAR).filter(|value| !value.is_empty())
    {
        return expand_tilde_path(PathBuf::from(value));
    }
    Ok(home_dir()?.join(".vibe"))
}
