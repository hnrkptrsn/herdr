...
fn parse_integration_target(
    ...
    let parsed = match target {
        ...
        "vibe" => IntegrationCommandTarget::Builtin(IntegrationTarget::Vibe),
        _ => {
            eprintln!("unknown integration target: {target}");
            eprintln!(
                "currently supported: pi, omp, claude, codex, copilot, devin, droid, kimi, opencode, kilo, hermes, qodercli, qwen, letta, cursor, mastracode, antigravity-cli, grok, vibe"
            );
            return Ok(None);
        }
    };
