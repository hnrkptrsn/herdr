---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and Mistral Vibe CLI.
---
...
herdr integration install vibe
```
...
herdr integration uninstall vibe
```
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI, Mistral Vibe CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Install the Mistral Vibe CLI hook:

```bash
herdr integration install vibe
```

The Vibe integration uses Vibe's native hook system. Herdr installs a `hooks.toml` configuration file that registers two hooks: `herdr-session-start` (triggered before each tool call) and `herdr-session-idle` (triggered after each agent turn completes). The hooks report session identity for native restore. Lifecycle state comes from Herdr's screen manifest detection.

Herdr uses `~/.vibe` by default, or `VIBE_CONFIG_DIR` when set. The Vibe config directory must already exist. Install writes `hooks.toml` in the Vibe config directory and creates `hooks/herdr-session-start` and `hooks/herdr-session-idle` hook scripts. Uninstall removes exactly those three Herdr-owned files.

After Vibe emits a session start event, Herdr can use the reported session id to resume the pane with `vibe --resume <id>`.
